...
async def startup_db_migration():
    ...
    try:
        ...
        # 5. Fix PostgreSQL identity columns for challenges and other SQLite-migrated tables
        db.execute(text("SET statement_timeout = 0")) # Ensure long alters don't timeout
        try:
            db.execute(text("ALTER TABLE weekly_challenges ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY"))
            db.execute(text("ALTER TABLE challenge_completions ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY"))
            db.execute(text("ALTER TABLE attendance_records ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY"))
            db.execute(text("ALTER TABLE activities ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY"))
            db.execute(text("ALTER TABLE post_likes ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY"))
            db.execute(text("ALTER TABLE exam_marks ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY"))
            db.commit()
            print("DEBUG: PostgreSQL identity fixes applied to all relevant tables")
        except Exception as e:
            db.rollback()
            print(f"DEBUG: PostgreSQL identity fix note (non-critical): {e}")
