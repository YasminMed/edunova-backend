...
async def startup_db_migration():
    ...
    try:
        ...
        # 5. Fix PostgreSQL sequences for tables that may be missing auto-increment
        # Uses CREATE SEQUENCE + SET DEFAULT instead of ADD IDENTITY, which works on existing tables with data
        tables_to_fix = [
            "weekly_challenges",
        ...
        for table in tables_to_fix:
            try:
                ...
                db.commit()
                print(f"DEBUG: Sequence fixed for {table}")
            except Exception as e:
                db.rollback()
                print(f"DEBUG: Sequence fix for {table}: {e}")
...
# uploads directory (kept for backward compat with any static files already there)
UPLOAD_DIR = os.path.abspath("uploads")
if not os.path.exists(UPLOAD_DIR):
    os.makedirs(UPLOAD_DIR)

# DB-backed file serving endpoint (persists across deploys unlike disk)
@app.get("/files/{file_id}")
async def serve_db_file(file_id: str, db: Session = Depends(get_db)):
    """Serve files stored in the database (UploadFile model). Persists across Railway redeploys."""
    upload = db.query(models.UploadFile).filter(models.UploadFile.id == file_id).first()
    if not upload:
...
async def create_post(
    ...
    try:
        ...
        image_url = None
        if image:
            # Store image in Postgres (UploadFile model) instead of disk
            # This ensures images persist across Railway redeploys (no ephemeral filesystem loss)
            ...
            print(f"DEBUG: Stored post image in DB with id={file_id}")
...
async def delete_post(post_id: int, db: Session = Depends(get_db)):
    ...
    # Delete associated image from DB if it was stored there
    if post.image_url and post.image_url.startswith("/files/"):
        file_id = post.image_url.split("/files/")[-1]
        db_file = db.query(models.UploadFile).filter(models.UploadFile.id == file_id).first()
        if db_file:
            db.delete(db_file)
    elif post.image_url and post.image_url.startswith("/uploads/"):
        # legacy disk file - try cleanup
...
# Duplicated block removed correctly. Logic is consolidated below.
...
async def get_weekly_challenge_status(email: str, db: Session = Depends(get_db)):
    ...
    challenge = db.query(models.WeeklyChallenge).filter(models.WeeklyChallenge.title == challenge_title).first()
    if not challenge:
        # Check if we have standard challenge seeding
        challenge = db.query(models.WeeklyChallenge).order_by(models.WeeklyChallenge.created_at.desc()).first()
        if not challenge:
            return {"has_challenge": False, "message": "No active challenge found"}
            
    # Calculate current progress
    quiz_count = db.query(models.QuizSubmission).filter(
        models.QuizSubmission.student_id == user.id,
        models.QuizSubmission.submitted_at >= week_start
    ).count()
    
    assignment_count = db.query(models.AssignmentSubmission).filter(
