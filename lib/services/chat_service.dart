...
class ChatService {
  ...
  Future<List<ChatMessage>> getChatMessages(
    ...
    try {
      final response = await _dio.get(
        ...
        queryParameters: {"current_user_email": currentUserEmail},
      );
      return (response.data as List)
  ...
  Future<List<GroupChat>> getUserGroupChats(String userEmail) async {
    try {
      ...
      return (response.data as List)
          .map((item) => GroupChat.fromJson(item))
          .toList();
    } catch (e) {
      return [];
    }
  ...
  Future<int?> createGroupChat(
    ...
    try {
      ...
      final response = await _dio.post("/groups", data: formData);
  ...
  Future<Map<String, dynamic>> getUserStatus(String email) async {
    try {
      final response = await _dio.get("/auth/user-status/$email");
      return response.data;
    } catch (e) {
      return {"is_online": false, "last_seen": DateTime.now().toIso8601String()};
    }
  }

  Future<void> sendHeartbeat(String email) async {
    try {
      await _dio.post("/auth/heartbeat/$email");
    } catch (e) {
      // ignore
    }
  }
